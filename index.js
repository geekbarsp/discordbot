...
import { AttachmentBuilder, ChannelType, Client, EmbedBuilder, GatewayIntentBits, Events, PermissionFlagsBits } from 'discord.js';
...
function getGuildConfigCandidateChannels(guild, preferredChannelId = null) {
  ...
  const preferredIds = [
    ...
    darkItemState.subscriptions?.[guild.id]?.channelId,
...
function getExistingGuildConfigChannel(guild) {
  return (
...
async function readGuildConfigMessageFromChannels(channels) {
  for (const channel of channels) {
    try {
      ...
      const configMessage = messages.find(
        (message) =>
      ...
      console.warn(`[GuildConfig] Could not scan ${channel.id} in guild ${channel.guild.id}: ${err.message}`);
    }
  }
...
async function readGuildConfigMessage(guild, preferredChannelId = null) {
  const configChannel = getExistingGuildConfigChannel(guild);
  const channels = [
    ...(configChannel ? [configChannel] : []),
    ...getGuildConfigCandidateChannels(guild, preferredChannelId),
  ];

  return readGuildConfigMessageFromChannels(channels);
}
...
async function persistGuildConfig(guild, preferredChannelId = null) {
  ...
  await saveGuildConfigToDiscord(guild, preferredChannelId);
}

async function deleteVisibleGuildConfigMessages(guild, keepMessageId = null, preferredChannelId = null) {
  const channels = getGuildConfigCandidateChannels(guild, preferredChannelId);
  ...
  for (const channel of channels) {
    try {
      ...
      for (const configMessage of configMessages.values()) {
        await configMessage.delete().catch(() => null);
      }
    } catch (err) {
...
async function handleTrollAction(message, action) {
  const targetUser = message.mentions.users.first();
  if (!targetUser) {
    return message.reply(`Usage: \`.${action} @user\``);
  }

  try {
    const png = await renderTrollActionImage(action, targetUser);
    ...
    console.error(`[TrollUser] ${action} render error:`, err);
    await message.reply(`I could not generate the ${action} image: ${err.message}`);
  }
}
...
async function handleJoinVoiceChannel(message) {
  ...
  await persistGuildConfig(message.guild, message.channel.id);
...
async function handleSyncga(message, args) {
  ...
  const existingSyncChannelId = darkItemState.syncChannels[message.guild.id];
  if (existingSyncChannelId) {
    const existingChannel = message.guild.channels.cache.get(existingSyncChannelId);
    return message.reply(`This server is already synced for global announcements in ${existingChannel ?? `#${existingSyncChannelId}`}.`);
  }
...
function getServerStatChannels(guild, categoryId) {
...
async function getAccurateMemberCounts(guild) {
...
function scheduleStatUpdate(guild, delayMs = 500) {
  const existing = pendingServerStatUpdates.get(guild.id);
  if (existing) {
  ...
  const timeout = setTimeout(async () => {
    ...
    try {
      await updateServerStatChannels(guild);
    } catch (err) {
...
// Keep alias so existing call-sites (ClientReady, GuildCreate) continue to work.
function scheduleServerStatsUpdate(guild, delayMs = 500) {
  scheduleStatUpdate(guild, delayMs);
}

async function updateServerStatChannels(guild) {
  const category = getServerStatCategory(guild);
  if (!category) {
    return;
  }

  const channels = getServerStatChannels(guild, category.id);
  ...
  let counts;
  try {
    counts = await getAccurateMemberCounts(guild);
  } catch (err) {
    console.error(`[ServerStats] Could not fetch member counts for guild ${guild.id}:`, err.message);
    return;
  }

  console.log(`[ServerStats] Updating guild ${guild.id}: total=${counts.totalMembers} users=${counts.userMembers} bots=${counts.botMembers} boosts=${counts.boosts}`);
    ...
    Object.entries(channels).map(async ([key, channel]) => {
      ...
      if (channel.name !== nextName) {
        try {
          ...
          console.error(`[ServerStats] Failed to rename channel ${channel.id} in guild ${guild.id}:`, err.message);
        }
      }
    }),
  );
...
async function handleServerStat(message) {
  ...
  try {
    ...
    const counts = await getAccurateMemberCounts(message.guild);
    const existingChannels = getServerStatChannels(message.guild, category.id);
    const definitions = ['totalMembers', 'userMembers', 'botMembers', 'boosts'];

    for (const key of definitions) {
      if (!existingChannels[key]) {
        await message.guild.channels.create({
          name: getServerStatChannelName(SERVER_STAT_CHANNEL_LABELS[key], counts[key]),
    ...
    await updateServerStatChannels(message.guild);
    await message.reply('✅ Server statistics channels are set up and will update in real-time.');
  } catch (err) {
...
client.on(Events.GuildMemberAdd, async (member) => {
  scheduleStatUpdate(member.guild);
});
...
client.on(Events.MessageCreate, async (message) => {
  ...
  switch (command) {
    ...
    case 'trolluserhelp':
      return handleTrollUserHelp(message);
    ...
    case 'kick':
    case 'throw':
      return handleTrollAction(message, command);
